 ...
 * be interned by the JVM, and stays resident until garbage collection. It will appear in heap dumps
 * taken while a {@code SigningConfig} is reachable, and the SDK cannot zero it. Treat the process
 * memory of anything holding a {@code SigningConfig} as key material, and prefer short-lived
 * signing processes over long-lived ones that keep the config alive.
 *
